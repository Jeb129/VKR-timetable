...
def init_group():
    inst, _ = Institute.objects.get_or_create(name="Институт")
    programm, _ = StudyProgram.objects.get_or_create(
        institute=inst, name="Программа", short_name="Прог"
    )
    return StudyGroup.objects.create(
        admission_year=22,
        study_program=programm,
        learning_form="o",
        learning_stage="b",
        group_num=1,
        name="Группа",
...
def test_m2m_override(redis_client):
    ...
    with draft_context(scenario, storage):
        draft = Lesson.objects.get(id=lesson.id)
        assert list(draft.study_groups.values_list("id", flat=True)) == [
            group1.id,
            group2.id,
        ]
