...
from api.serializers.infrastructure import ClassroomSerializer
...
class LessonViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self) -> BaseManager[Lesson]:
        queryset = (
            Lesson.objects.all()
        ...
        if date_str:
            try:
                ...
                print(f"\n--- ФИЛЬТРАЦИЯ ---")
                print(
                    f"Дата: {date_str} | День недели: {day_of_week} | Неделя года: {week_number}"
                )
                print(
                    f"Определено как: {'ЧИСЛИТЕЛЬ' if current_week_num == 1 else 'ЗНАМЕНАТЕЛЬ'} (week_num={current_week_num})"
                )
                print(f"------------------\n")
