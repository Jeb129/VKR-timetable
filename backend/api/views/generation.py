...
from api.models import ScheduleScenario
...
class GenerationViewSet(viewsets.ViewSet):
    ...
    def stop(self, request, scenario_id=None):
        ...
        if success:
            return Response({"status": "stopping", "message": "Подан сигнал на остановку"})
        return Response({"error": "Задача не запущена или уже завершена"}, status=status.HTTP_400_BAD_REQUEST)
