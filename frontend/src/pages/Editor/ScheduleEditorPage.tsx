...
import type { Timeslot, Lesson } from "@/types/schedule";
...
const ScheduleEditorPage = () => {
    ...
    // 1. Загрузка справочника таймслотов
    useEffect(() => {
        dbService.list<Timeslot>("timeslots",{page_size:100}).then(res => setTimeslots(res.results));
    }, []);

    // 2. Логика перетаскивания (DND)
    ...
    // 4. Модалки (Удаление, Создание, Редактирование)
    ...
    return (
        <div className="flex-col bg-main min-h-screen">
            <nav className="navbar">
                <div className="logo-white pointer" onClick={() => navigate("/")}>КГУ • РЕДАКТОР</div>
            ...
            <div className="editor-container">
                <div className="grid-area flex-col gap-2 p-2">
                    <div className="card flex-row gap-2 align-end p-2">
                        <div className="flex-col" style={{ width: '200px' }}>
                            ...
                            <select className="select-styled" value={filterType} onChange={e => {
                ...
                <div className={`error-sidebar ${isSidebarOpen ? '' : 'closed'}`}>
                    <div className="sidebar-tabs flex-row">
                        ...
                        <button className={`tab-btn f-1 p-1 ${activeTab === 'changes' ? 'active' : ''}`} onClick={() => setActiveTab('changes')}>
                            Изменения ({draftChanges.length})
                        </button>
                        <button className={`tab-btn f-1 p-1 ${activeTab === 'trash' ? 'active' : ''}`} onClick={() => setActiveTab('trash')}>
                            Корзина ({deletedLessons.length})
                        </button>
                    </div>

                    <div className="flex-col scroll-y f-1 p-1 gap-1">
                        {loading ? <div className="p-2 text-center">Загрузка...</div> : (
                            <>
                                {activeTab === 'errors' && lessonErrors.map((val, i) => (
                                    <LessonErrorItem key={i} lesson={val.lesson} errors={val.errors} />
                                ))}
                                {activeTab === 'changes' && draftChanges.map((lesson) => (
                                    <ChangeLogItem key={lesson.id} lesson={lesson} onRevert={revertLesson} />
                                ))}
                                {activeTab === 'trash' && deletedLessons.map(l => (
                                    <DeletedLogItem key={l.id} lesson={l} onRestore={handleRestore} />
                                ))}
                            </>
                        )}
                    </div>
